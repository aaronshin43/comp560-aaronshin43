...
- Write `gen_data.py`:
  - Phase 1: plain 3/4-digit and scratchpad 3/4-digit generators
  - Phase 2: repeated-prefix scratchpad generator (multipliers 1–5)
- Run `prepare.py` on each variant

### Step 2: Training
- Write configs for each new condition under `config/`
- Training commands go in `COMMANDS.md`
- Use `train_benchmark.py` with `always_save_checkpoint=True` and named snapshots

### Step 3: Evaluation
- Run `eval_generation.py` on final checkpoint for each condition
- Collect results into `results/accuracy_phase1.csv` and `results/accuracy_phase2.csv`

### Step 4: Analysis
- Phase 1: bar chart of masking gap vs (format × digit_length)
- Phase 2: line plot of masking gap vs input fraction
